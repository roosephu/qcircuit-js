...
(function() {
  var Axis, ECcnt, ECs, Q, QC, Qcircuit, QcircuitGrid, Qcircuit_black_dot, Qcircuit_component, Qcircuit_gate, Qcircuit_label, Qcircuit_line, Qcircuit_measure, Qcircuit_measureD, Qcircuit_measuretab, Qcircuit_meter, Qcircuit_multigate, Qcircuit_multimeasure, Qcircuit_multimeasureD, Qcircuit_qswap, Qcircuit_target, Qcircuit_white_dot, Qcircuit_wire, QueueEvent, ReFmt, ReFmt_col_add, ReFmt_col_del, ReFmt_row_add, ReFmt_row_del, X, Y, ad, argc, bind_RC_alter, bind_button, btn, btn_group_EC, btn_group_RC, center, click_event, clog, cols, dashed_box, draw, drawer, elec_elem, get_cur_rel_pos, insert_tab, locate_mouse, mk_table, rc, rows, sz_cfg, _i, _j, _k, _len, _len1, _len2, _ref, _ref1, _ref2;
  ...
  Axis = (function() {
    ...
    Axis.prototype.upd_xs = function() {
      var cnt, i, x, _i, _j, _len, _ref, _ref1, _results;
      this.last = 0;
      this.xs = [];
      this.rep = [];
      cnt = 0;
  ...
  ReFmt_row_add = (function() {
    function ReFmt_row_add(pos) {
      this.pos = pos;
      console.log("drd");
    }

    ReFmt_row_add.prototype.point = function(x, y) {
      if (x >= this.pos) {
        return [x + 1, y];
      } else {
        return [x, y];
    ...
    ReFmt_row_add.prototype.line = function(x1, y1, x2, y2) {
      return this.point(x1, y1).concat(this.point(x2, y2));
    };

    return ReFmt_row_add;

  })();

  ReFmt_col_add = (function() {
    function ReFmt_col_add(pos) {
      this.pos = pos;
    }

    ReFmt_col_add.prototype.point = function(x, y) {
      if (y >= this.pos) {
        return [x, y + 1];
      } else {
        return [x, y];
    ...
    ReFmt_col_add.prototype.line = function(x1, y1, x2, y2) {
      return this.point(x1, y1).concat(this.point(x2, y2));
    };

    return ReFmt_col_add;

  })();

  ReFmt_row_del = (function() {
    function ReFmt_row_del(pos) {
      this.pos = pos;
    }

    ReFmt_row_del.prototype.point = function(x, y, d) {
      if (d == null) {
        d = true;
      }
      if (x === this.pos && d) {
    ...
    ReFmt_row_del.prototype.line = function(x1, y1, x2, y2) {
      var _ref, _ref1;
      if (x1 === x2 && x1 === this.pos) {
        return false;
      }
      _ref = this.point(x1, y1, false), x1 = _ref[0], y1 = _ref[1];
      _ref1 = this.point(x2, y2, false), x2 = _ref1[0], y2 = _ref1[1];
      if ([x1, y1] === [x2, y2]) {
        return false;
      }
      return [x1, y1, x2, y2];
    ...
    return ReFmt_row_del;

  })();

  ReFmt_col_del = (function() {
    function ReFmt_col_del(pos) {
      this.pos = pos;
    }

    ReFmt_col_del.prototype.point = function(x, y, d) {
      if (d == null) {
        d = true;
      }
      if (y === this.pos && d) {
    ...
    ReFmt_col_del.prototype.line = function(x1, y1, x2, y2, gate) {
      var _ref, _ref1;
      if (y1 === y2 && y1 === this.pos) {
        return false;
      }
      _ref = this.point(x1, y1, false), x1 = _ref[0], y1 = _ref[1];
      _ref1 = this.point(x2, y2, false), x2 = _ref1[0], y2 = _ref1[1];
    ...
    return ReFmt_col_del;

  })();

  ReFmt = function(mode, pos) {
    switch (mode) {
      case "ra":
        return new ReFmt_row_add(pos);
      case "rd":
        return new ReFmt_row_del(pos);
      case "ca":
        return new ReFmt_col_add(pos);
      case "cd":
        return new ReFmt_col_del(pos);
    }
  };

  Qcircuit_black_dot = (function() {
    function Qcircuit_black_dot(c, x, y) {
      this.c = c;
      this.x = x;
      this.y = y;
      this.type = 'black-dot';
    ...
    Qcircuit_black_dot.prototype.draw = function(svg) {
      var cc, rad, xc, yc, _ref, _ref1;
      rad = sz_cfg['circle'] / 2;
      _ref = center(this.x, this.c), xc = _ref[0], cc = _ref[1];
      _ref1 = center(this.y, this.c), yc = _ref1[0], cc = _ref1[1];
      this.dom = svg.group();
      svg.line(cc, xc, cc, yc).addTo(this.dom).stroke({
        width: 1
      });
      svg.circle(rad * 2).addTo(this.dom).move(cc - rad, xc - rad);
      if (!this.tab) {
        return this.tab = insert_tab(this, this.cid, "black-dot", "" + this.c + " " + this.x + " " + this.y);
      }
    };

    Qcircuit_black_dot.prototype.apply = function(map) {
      return map[this.x][this.c] += "\\ctrl{" + (this.y - this.x) + "}";
    };

    Qcircuit_black_dot.prototype.refmt = function(mode) {
      var d, ret, _ref;
      ret = mode.point(this.x, this.c);
      if (!ret) {
        return false;
      }
      return _ref = mode.line(this.x, this.c, this.y, this.c), this.x = _ref[0], this.c = _ref[1], this.y = _ref[2], d = _ref[3], _ref;
    };
  ...
  Qcircuit_white_dot = (function() {
    function Qcircuit_white_dot(c, x, y) {
      this.c = c;
      this.x = x;
      this.y = y;
      this.type = 'white-dot';
    ...
    Qcircuit_white_dot.prototype.draw = function(svg) {
      ...
      svg.circle(rad * 2).addTo(this.dom).move(cc - rad, xc - rad).attr({
        'stroke-width': 2,
        'fill': 'white',
        'fill-opacity': 1
      });
      if (!this.tab) {
        return this.tab = insert_tab(this, this.cid, "white-dot", "" + this.c + " " + this.x + " " + this.y);
      }
    };

    Qcircuit_white_dot.prototype.apply = function(map) {
      return map[this.x][this.c] += "\\ctrlo{" + (this.y - this.x) + "} ";
    };

    Qcircuit_white_dot.prototype.refmt = function(mode) {
      var d, ret, _ref;
      ret = mode.point(this.x, this.c);
      if (!ret) {
        return false;
  ...
  Qcircuit_target = (function() {
    ...
    Qcircuit_target.prototype.refmt = function(mode) {
      var ret;
      ret = mode.point(this.x, this.y);
      if (!ret) {
        return false;
      }
      return this.x = ret[0], this.y = ret[1], ret;
    };
  ...
  Qcircuit_line = (function() {
    ...
    Qcircuit_line.prototype.refmt = function(mode) {
      var ret;
      ret = mode.line(this.x1, this.y1, this.x2, this.y2);
      if (!ret) {
        return false;
      }
      return this.x1 = ret[0], this.y1 = ret[1], this.x2 = ret[2], this.y2 = ret[3], ret;
    };
  ...
  Qcircuit_qswap = (function() {
    ...
    Qcircuit_qswap.prototype.refmt = function(mode) {
      var ret;
      ret = mode.point(this.x, this.y);
      if (!ret) {
        return false;
  ...
  Qcircuit_gate = (function() {
    ...
    Qcircuit_gate.prototype.refmt = function(mode) {
      var ret;
      ret = mode.point(this.x, this.y);
      if (!ret) {
        return false;
  ...
  Qcircuit_multigate = (function() {
    ...
    Qcircuit_multigate.prototype.refmt = function(mode) {
      var ret, _;
      ret = mode.line(this.x, this.c, this.y, this.c);
      if (!ret) {
        return false;
      }
      return this.x = ret[0], this.c = ret[1], this.y = ret[2], _ = ret[3], ret;
    };
  ...
  Qcircuit_label = (function() {
    ...
    Qcircuit_label.prototype.refmt = function(mode) {
      var ret;
      ret = mode.point(this.x, this.y);
      if (!ret) {
        return false;
  ...
  Qcircuit_wire = (function() {
    ...
    Qcircuit_wire.prototype.refmt = function(mode) {
      var ret;
      ret = mode.line(this.x1, this.y1, this.x2, this.y2);
      if (!ret) {
        return false;
  ...
  Qcircuit_meter = (function() {
    ...
    Qcircuit_meter.prototype.refmt = function(mode) {
      var ret;
      ret = mode.point(this.x, this.y);
      if (!ret) {
        return false;
  ...
  Qcircuit_measure = (function() {
    ...
    Qcircuit_measure.prototype.refmt = function(mode) {
      var ret;
      ret = mode.point(this.x, this.y);
      if (!ret) {
        return false;
  ...
  Qcircuit_measuretab = (function() {
    ...
    Qcircuit_measuretab.prototype.refmt = function(mode) {
      var ret;
      ret = mode.point(this.x, this.y);
      if (!ret) {
        return false;
  ...
  Qcircuit_measureD = (function() {
    ...
    Qcircuit_measureD.prototype.draw = function(svg) {
      ...
      svg.path("").addTo(this.dom).attr({
        ...
        "fill": "white",
        "fill-opacity": 0,
        "d": "M " + (yc - 20) + " " + (xc - 12) + "                              L " + (yc + 12) + " " + (xc - 12) + "                              A " + 12 + " " + 12 + " 90 0 1 " + (yc + 12) + " " + (xc + 12) + "                              L " + (yc - 20) + " " + (xc + 12) + "                              Z"
      });
      if (this.txt !== "") {
    ...
    Qcircuit_measureD.prototype.refmt = function(mode) {
      var ret;
      ret = mode.point(this.x, this.y);
      if (!ret) {
        return false;
  ...
  Qcircuit_multimeasure = (function() {
    function Qcircuit_multimeasure(c, x, y, txt) {
      var _ref;
      this.c = c;
      this.x = x;
      this.y = y;
      ...
      this.type = 'multimeasure';
    }

    Qcircuit_multimeasure.prototype.draw = function(svg) {
      var d, lc, r, uc, xc;
      d = sz_cfg['gate'] / 2;
      r = 12;
      xc = Y.center(this.c);
      ...
      svg.path("").addTo(this.dom).attr({
        ...
        "fill": "white",
        "fill-opacity": 0,
        "d": "M " + (xc - r) + " " + (uc - r) + "                               A " + r + " " + r + " 90 0 0 " + (xc - r * 2) + " " + uc + "                              L " + (xc - r * 2) + " " + lc + "                              A " + r + " " + r + " 90 0 0 " + (xc - r) + " " + (lc + r) + "                              L " + (xc + r) + " " + (lc + r) + "                              A " + r + " " + r + " 90 0 0 " + (xc + r * 2) + " " + lc + "                              L " + (xc + r * 2) + " " + uc + "                              A " + r + " " + r + " 90 0 0 " + (xc + r) + " " + (uc - r) + "                              Z"
      });
      if (this.txt !== "") {
        svg.image("http://frog.isima.fr/cgi-bin/bruno/tex2png--10.cgi?" + this.txt, d, d).addTo(this.dom).move(xc - 10, (lc + uc) / 2 - 10);
      }
      if (!this.tab) {
        return this.tab = insert_tab(this, this.cid, "multimeasure", "" + this.c + " " + this.x + " " + this.y + " " + this.txt);
      }
    };

    Qcircuit_multimeasure.prototype.apply = function(map) {
      var d, _i, _ref, _ref1, _results;
      map[this.x][this.c] += "\\multimeasure{" + (this.y - this.x) + "}{" + this.txt + "}";
      _results = [];
      for (d = _i = _ref = this.x + 1, _ref1 = this.y; _ref <= _ref1 ? _i <= _ref1 : _i >= _ref1; d = _ref <= _ref1 ? ++_i : --_i) {
        _results.push(map[d][this.c] += "\\ghost{" + this.txt + "}");
      }
    ...
    Qcircuit_multimeasure.prototype.refmt = function(mode) {
      var ret, _;
      ret = mode.line(this.x, this.c, this.y, this.c);
      if (!ret) {
        return false;
    ...
    return Qcircuit_multimeasure;

  })();

  Qcircuit_multimeasureD = (function() {
    function Qcircuit_multimeasureD(c, x, y, txt) {
      var _ref;
      this.c = c;
      this.x = x;
      this.y = y;
      ...
      this.type = 'multimeasureD';
    }

    Qcircuit_multimeasureD.prototype.draw = function(svg) {
      var d, lc, r, uc, xc;
      d = sz_cfg['gate'] / 2;
      r = 12;
      xc = Y.center(this.c);
      ...
      svg.path("").addTo(this.dom).attr({
        ...
        "fill": "white",
        "fill-opacity": 0,
        "d": "M " + (xc - d) + " " + (uc - r) + "                               L " + (xc - d) + " " + (lc + r) + "                              L " + (xc + r) + " " + (lc + r) + "                              A " + r + " " + r + " 90 0 0 " + (xc + r * 2) + " " + lc + "                              L " + (xc + r * 2) + " " + uc + "                              A " + r + " " + r + " 90 0 0 " + (xc + r) + " " + (uc - r) + "                              Z"
      });
      if (this.txt !== "") {
        svg.image("http://frog.isima.fr/cgi-bin/bruno/tex2png--10.cgi?" + this.txt, d, d).addTo(this.dom).move(xc - 10, (lc + uc) / 2 - 10);
      }
      if (!this.tab) {
        return this.tab = insert_tab(this, this.cid, "multimeasureD", "" + this.c + " " + this.x + " " + this.y + " " + this.txt);
      }
    };

    Qcircuit_multimeasureD.prototype.apply = function(map) {
      var d, _i, _ref, _ref1, _results;
      map[this.x][this.c] += "\\multimeasureD{" + (this.y - this.x) + "}{" + this.txt + "}";
      _results = [];
      for (d = _i = _ref = this.x + 1, _ref1 = this.y; _ref <= _ref1 ? _i <= _ref1 : _i >= _ref1; d = _ref <= _ref1 ? ++_i : --_i) {
        _results.push(map[d][this.c] += "\\ghost{" + this.txt + "}");
      }
    ...
    Qcircuit_multimeasureD.prototype.refmt = function(mode) {
      var ret, _;
      ret = mode.line(this.x, this.c, this.y, this.c);
      if (!ret) {
        return false;
  ...
  Qcircuit_component = (function() {
    ...
    Qcircuit_component.prototype.refmt = function(mode, pos) {
      var c, id, refmt, _ref;
  ...
  bind_button = function(Qcircuit, btn, mode) {
    var argc, func;
    func = (function() {
      switch (mode) {
        case 1:
          return function(arg) {
            ...
            return QC.add(new Qcircuit(x, y, $('#gate').val()));
          };
        case 2:
          return function(arg) {
            ...
            if (y1 === y2 && x1 !== x2) {
              return QC.add(new Qcircuit(y1, x1, x2, $('#gate').val()));
            }
          };
        case 3:
          return function(arg) {
            ...
            _ref = arg[0], x1 = _ref[0], y1 = _ref[1];
            _ref1 = arg[1], x2 = _ref1[0], y2 = _ref1[1];
            if ((y1 === y2 || x1 === x2) && ([x1, y1] !== [x2, y2])) {
              return QC.add(new Qcircuit(x1, y1, x2, y2));
            }
          };
        case 4:
          return function(arg) {
            ...
            return QC.add(new Qcircuit(x, y, io, dirac, $('#gate').val()));
          };
      }
    })();
    argc = mode === 1 ? 1 : 2;
    ...
    return $("#btn-" + btn).click(function() {
      return Q.bind(func, argc);
    });
  };

  elec_elem = [[Qcircuit_target, 'target', 1], [Qcircuit_qswap, 'qswap', 1], [Qcircuit_gate, 'gate', 1], [Qcircuit_meter, 'meter', 1], [Qcircuit_measure, 'measure', 1], [Qcircuit_measuretab, 'measuretab', 1], [Qcircuit_measureD, 'measureD', 1], [Qcircuit_multigate, 'multigate', 2], [Qcircuit_multimeasure, 'multimeasure', 2], [Qcircuit_multimeasureD, 'multimeasureD', 2], [Qcircuit_black_dot, 'black-dot', 2], [Qcircuit_white_dot, 'white-dot', 2], [Qcircuit_line, 'line', 3], [Qcircuit_wire, 'wire', 3], [Qcircuit_label, 'label', 4]];

  for (_i = 0, _len = elec_elem.length; _i < _len; _i++) {
    _ref = elec_elem[_i], Qcircuit = _ref[0], btn = _ref[1], argc = _ref[2];
    bind_button(Qcircuit, btn, argc);
  }

  btn_group_RC = $("#btn-group-RC");

  bind_RC_alter = function(mode, txt) {
    btn_group_RC.append("<button class='btn btn-primary' id='btn-RC-" + mode + "'>" + txt + "</button>");
    return $("#btn-RC-" + mode).click(function() {
      return QC.refmt(mode, parseInt($("#gate").val()));
    });
  };
  ...
  QcircuitGrid = (function() {
    function QcircuitGrid(rows, cols) {
      var i, j, _l, _m, _ref3, _ref4;
      this.rows = rows;
      this.cols = cols;
      this.map = [];
      for (i = _l = 1, _ref3 = this.rows; 1 <= _ref3 ? _l <= _ref3 : _l >= _ref3; i = 1 <= _ref3 ? ++_l : --_l) {
        this.map[i] = [];
        for (j = _m = 1, _ref4 = this.cols; 1 <= _ref4 ? _m <= _ref4 : _m >= _ref4; j = 1 <= _ref4 ? ++_m : --_m) {
          this.map[i][j] = ' & ';
        }
      }
    }
    ...
    QcircuitGrid.prototype.exp_tex = function() {
      var comp, id, ret, x, y, _l, _m, _ref3, _ref4, _ref5, _ref6;
      _ref3 = this.components;
      for (id in _ref3) {
        comp = _ref3[id];
        if (comp.type !== 'line') {
      ...
      for (x = _l = 1, _ref5 = this.rows; 1 <= _ref5 ? _l <= _ref5 : _l >= _ref5; x = 1 <= _ref5 ? ++_l : --_l) {
        for (y = _m = 1, _ref6 = this.cols; 1 <= _ref6 ? _m <= _ref6 : _m >= _ref6; y = 1 <= _ref6 ? ++_m : --_m) {
          ret += this.map[x][y];
        }
        ret += "\\\\ \n";
      }
  ...
  mk_table = function() {
    var elem, h, i, inner, j, rem, s, style, tab, w, _l, _m;
    tab = $("#table");
    rem = 20;
    for (i = _l = 0; 0 <= rows ? _l <= rows : _l >= rows; i = 0 <= rows ? ++_l : --_l) {
      h = i === 0 ? rem : X.get(i);
      s = "<tr height=" + h + "px>";
      for (j = _m = 0; 0 <= cols ? _m <= cols : _m >= cols; j = 0 <= cols ? ++_m : --_m) {
        elem = i === 0 ? "th" : "td";
