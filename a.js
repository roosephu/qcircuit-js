...
(function() {
  var Axis, Q, QC, QCircuitGrid, QCircuit_black_dot, QCircuit_component, QCircuit_gate, QCircuit_line, QCircuit_multigate, QCircuit_qswap, QCircuit_target, QCircuit_white_dot, QueueEvent, X, Y, c, center, clog, cols, dashed_box, draw, drawer, export_to_latex, get_cur_rel_pos, locate_mouse, mk_table, rows, sz_cfg, _i, _len, _ref;
  ...
  QCircuit_black_dot = (function() {
    function QCircuit_black_dot(x, y) {
      this.x = x;
      this.y = y;
      this.type = 'black-dot';
    }
    ...
    QCircuit_black_dot.prototype.draw = function(svg) {
      ...
      clog("black-dot " + xc + " " + yc + " " + this.x + " " + this.y);
  ...
  QCircuit_white_dot = (function() {
    function QCircuit_white_dot(x, y) {
      this.x = x;
      this.y = y;
      this.type = 'white-dot';
    }
  ...
  QCircuit_component = (function() {
    ...
    QCircuit_component.prototype.fix_cover = function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        c = _ref[_i];
        if ((_ref1 = c.type) === 'black-dot' || _ref1 === 'white-dot' || _ref1 === 'targ' || _ref1 === 'gate' || _ref1 === 'multigate') {
  ...
  _ref = [new QCircuit_line(1, 1, 3, 1), new QCircuit_line(1, 2, 4, 2), new QCircuit_line(1, 3, 3, 3), new QCircuit_black_dot(1, 1), new QCircuit_black_dot(2, 2), new QCircuit_black_dot(3, 2), new QCircuit_white_dot(3, 3), new QCircuit_black_dot(4, 2), new QCircuit_target(1, 2), new QCircuit_target(3, 1), new QCircuit_gate(1, 3, 'U'), new QCircuit_qswap(4, 1)];
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    c = _ref[_i];
    QC.add(c, false);
  }
  ...
  window.add_black_dot = function() {
    ...
    func = function(arg) {
      ...
      return QC.add(new QCircuit_black_dot(x, y));
    };
  ...
  window.add_white_dot = function() {
    ...
    func = function(arg) {
      var x, y, _ref1;
      _ref1 = arg[0], x = _ref1[0], y = _ref1[1];
      return QC.add(new QCircuit_white_dot(x, y));
    };
    return Q.bind(func, 1);
  };
  ...
  QCircuitGrid = (function() {
    function QCircuitGrid(rows, cols) {
      ...
      this.map = [
        (function() {
          var _j, _results;
          _results = [];
          for (j = _j = 1; 1 <= rows ? _j <= rows : _j >= rows; j = 1 <= rows ? ++_j : --_j) {
            _results.push([
              (function() {
                var _k, _results1;
                _results1 = [];
                for (i = _k = 1; 1 <= cols ? _k <= cols : _k >= cols; i = 1 <= cols ? ++_k : --_k) {
                  _results1.push(new GridNode('null'));
                }
                return _results1;
              })()
            ]);
    ...
    QCircuitGrid.prototype.imp_ops = function(ops) {
      var cmd, op, x, y, _j, _len1, _ref1, _results;
      _results = [];
      for (_j = 0, _len1 = ops.length; _j < _len1; _j++) {
        op = ops[_j];
        cmd = op[0];
        if (cmd === 'black-dot') {
          _results.push((_ref1 = op.slice(1, 4), x = _ref1[0], y = _ref1[1], _ref1));
        } else {
          _results.push(void 0);
        }
      }
